...
- `AUTH/LOGOUT` 当前连接未登录：`2001 AUTH_REQUIRED`
- `AUTH/LOGOUT` token 无效或与当前连接不匹配：`2002 TOKEN_INVALID`
- `AUTH/REGISTER` 用户名或邮箱冲突：`2006 REGISTER_CONFLICT`
- `AUTH/REGISTER` 数据库写入失败：`2007 REGISTER_FAILED`
- `PROFILE` 数据字段校验失败：`3003 PROFILE_VALIDATION_FAILED`
- `PROFILE/ADD_FRIEND` 目标用户不存在：`3001 PROFILE_NOT_FOUND`
- `PROFILE/ADD_FRIEND` 关系冲突（如拉黑）：`3002 PROFILE_UPDATE_CONFLICT`
- `PROFILE/ADD_FRIEND` 数据库失败：`1099 INTERNAL_ERROR`
- `PROFILE/DELETE_FRIEND` 数据字段校验失败：`3003 PROFILE_VALIDATION_FAILED`
- `PROFILE/DELETE_FRIEND` 任一用户不存在：`3001 PROFILE_NOT_FOUND`
- `PROFILE/DELETE_FRIEND` 数据库失败：`1099 INTERNAL_ERROR`
